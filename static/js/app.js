...
let currentTypeFilter = 'all'; // 当前歌曲类型过滤
...
function applyFilters() {
    ...
    if (currentTypeFilter && currentTypeFilter !== 'all') {
        songs = songs.filter(s => s.type === currentTypeFilter);
    }
...
function updateFilterBadges() {
    ...
    document.querySelectorAll('.filter-tabs:not(.type-filter-tabs) .filter-tab').forEach(tab => {
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('.filter-tabs:not(.type-filter-tabs) .filter-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            document.querySelectorAll('.filter-tabs:not(.type-filter-tabs) .filter-tab').forEach(t => t.classList.remove('active'));
    ...
    // 歌曲类型过滤标签
    document.querySelectorAll('.type-filter-tabs .filter-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            document.querySelectorAll('.type-filter-tabs .filter-tab').forEach(t => t.classList.remove('active'));
            tab.classList.add('active');
            currentTypeFilter = tab.dataset.typeFilter;
            applyFilters();
        });
    });
